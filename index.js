...
const router = async () => {
  const routes = [
    { path: "/demos/blog/"},
    { path: "/demos/blog/dev"},
    { path: "/demos/blog/else"},
    { path: "/demos/blog/about"},
    { path: "/demos/blog/contact"},
    { path: "/demos/blog/contact.php"},
  ];
  ...
  if (match.route === routes[0] || match.route === routes[1] || match.route === routes[2]) {
    ...
    if (location.pathname.replace("/demos/blog", "").substring(1) !=="" && document.getElementById(location.pathname.replace("/demos/blog", "").substring(1))) {
      let postsArr = document.getElementsByClassName('cat');
      for (let i=0; i<postsArr.length; i++){
        postsArr[i].style.display = 'none';
      }
      document.getElementById(location.pathname.replace("/demos/blog", "").substring(1) + '-preview').style.display = 'none';
      document.getElementById(location.pathname.replace("/demos/blog", "").substring(1) + '-body').style.display = 'block';
      document.getElementById(location.pathname.replace("/demos/blog", "").substring(1) + '-button').style.display = 'none';
      document.getElementById(location.pathname.replace("/demos/blog", "").substring(1)).style.display = 'block';
    }
    else {
